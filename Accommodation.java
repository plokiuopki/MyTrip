...
public class Accommodation {
    ...
    // to sort hotels based on user choice
    private void sortHotels(List<Hotel> hotels) {
        System.out.println("Display hotels by:\n1. Price\n2. Distance\n3. Review Rating");
        int sortChoice = scanner.nextInt();

        switch (sortChoice) {
            case 1 -> hotels.sort(Comparator.comparingDouble(Hotel::getPrice));
            case 2 -> hotels.sort(Comparator.comparingDouble(Hotel::getDistance));
            case 3 -> hotels.sort(Comparator.comparingDouble(Hotel::getReview).reversed());
            default -> System.out.println("Invalid sorting option. Displaying unsorted hotels.");
        }
    }
    ...
    public Hotel calculateHotel(String destination) {
        ...
        // Sort hotels based on user’s sorting preference
        sortHotels(hotels);
